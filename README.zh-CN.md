...
也可以从 stdin 接收已批准 spec 派生出的编程 Agent 或自动化系统输出：

```bash
ai-planner --from-spec harness/specs/SPEC-0001/spec.md | python3 -m attestflow task import --from-json - --from-spec harness/specs/SPEC-0001/spec.md --approve
```

`task import` 会先校验 approved spec provenance，再分配 `TASK-*` ID、解析 planner 内部依赖、补齐默认字段、校验 ready 门禁，并写入 `harness/tasks/ready/*.json`。如果编程 Agent 输出缺少 scope、BDD、unit_tests、acceptance 或 files.write，导入会返回非零并拒绝写入半成品任务。
...
`source import` 会把 GitHub issue、Linear/Jira ticket、PR review comment 或 CI failure JSON 快照保存到 `harness/sources/.../source.json`，并创建 `harness/tasks/proposed/TASK-*.json`。这些 proposed task 保留 `source.kind`、外部 ID、URL、优先级和证据路径；真正的 ready task 必须先由 source evidence 收敛成 approved spec，再由 planner 生成，避免把外部 ticket 直接当成可执行实现边界。

内置 planner capability 仍用于 approved spec 主路径和内部受控修复路径。raw goal 不能直接调用 `plan`；请从 `go <requirement source>` 开始：

```bash
python3 -m attestflow go "实现登录功能"
python3 -m attestflow go --from-spec harness/specs/SPEC-0001/spec.md --approve
```
...
接入后先让编程 Agent 审核 `harness.yml` 和项目命令，再用 `go <requirement source>` 生成 draft spec、完成 clarification、批准 spec，然后由 approved spec 生成 planner JSON 并导入任务。只有凭证、业务取舍和不可自动判断的外部决策需要人工确认。任务进入开发前必须满足 `ready` 门禁；完成前必须有当前 run 的 evidence。
...
`autopilot --run --limit N --max-steps M` 是已有 ready/active task 的顶层执行入口。它会创建 `harness/autopilot-runs/<run>/metadata.json` 和 `ledger.jsonl`，按 dry-run 同一套规则优先批量推进已开始任务的下一步动作，再分发新的 ready 批次。`autopilot --run --goal` 已被拒绝；raw goal/PRD 必须先通过 `go <requirement source>` 生成并批准 spec。当前 run 能自动完成“active action batch or dispatch -> ledger”：`in_progress` 会按 `bdd -> tdd -> implementer -> review` 推进，`review` 会按 `reviewer -> optional verifier -> verify` 推进，`verified` 会进入 `accepted`，`accepted` 会先把 worktree 变更合回控制仓库，再执行已配置的 `publish`，随后执行已配置的 `pr ensure`、采集 CI evidence。release failure repair planner 是内部受控路径，会携带内部 provenance，不对 CLI 暴露 raw goal 绕过开关。`--max-steps` 到点且仍有 active task、ready batch 或 release gate 时，run 会写成 `status: paused` 与 `pause_reason: max_steps_reached`，表示可用 `--resume` 继续，而不是已经完成。`--loop` 会在同一个 top-level run 上受限地反复 resume paused run，直到 `finished`、`blocked`、`failed` 或 cycle 用完。
...
- `go --from-spec SPEC --approve` 调用 planner capability 生成并导入 runtime task JSON，然后进入同一轮自治执行
